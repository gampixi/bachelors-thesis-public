...
def _load_detections(detection_path) -> list[int]:
...
def _update_detections_string():
    global current_detections
    pos_entry.delete(0, tkinter.END)
    pos_entry.insert(0, "")
    for imp in current_detections:
...
def _load_current_record():
    ...
    global current_samples, current_detections
    ...
    current_detections = _load_detections(records[current_record_idx]['detection_path'])
    _plot_current_samples()
    _process_minigolf()
    _update_detections_string()


def _deactivate_record():
    global records, current_record_idx
    new_name = f"{records[current_record_idx]['data_path']}.deactivated"
    print(f"Renaming {records[current_record_idx]['data_path']} to {new_name}")
    os.rename(records[current_record_idx]['data_path'], new_name)
...
def _save_current_sample(isNot: bool, swingType: SwingType | None,
    ...
    new_data_name = records[current_record_idx]['data_path'].replace(f'e2e_dataset/{dataset_kind}',
                                                                     f'e2e_dataset/{folder_name}')
    new_cal_name = records[current_record_idx]['calibration_path'].replace(f'e2e_dataset/{dataset_kind}',
                                                                           f'e2e_dataset/{folder_name}')
...
def main():
    global current_samples, window, fig, ax_accel_arm, ax_gyro_arm, ax_accel_palm, ax_gyro_palm, canvas, mgresult, dataset_kind
    ...
    window = tkinter.Tk()
    window.title('Validācijas datu kopas pārlūks')
    ...
    quit_button = tkinter.Button(master=window, text="Iziet", command=_quit)
    ...
    deactivate_file_button = tkinter.Button(master=snippet_frame, text="Deaktivizēt failu", command=_deactivate_record)
    ...
    save_as_fs_right_off_btn = tkinter.Button(master=detector_grid_frame, text="FS / RIGHT / OFF",
                                              command=_save_fs_right_off)
    save_as_fs_left_off_btn = tkinter.Button(master=detector_grid_frame, text="FS / LEFT / OFF",
                                             command=_save_fs_left_off)
    save_as_fs_right_dm_btn = tkinter.Button(master=detector_grid_frame, text="FS / RIGHT / DOMIN.",
                                             command=_save_fs_right_dm)
    save_as_fs_left_dm_btn = tkinter.Button(master=detector_grid_frame, text="FS / LEFT / DOMIN.",
                                            command=_save_fs_left_dm)
    save_as_put_right_off_btn = tkinter.Button(master=detector_grid_frame, text="PUT / RIGHT / OFF",
                                               command=_save_put_right_off)
    save_as_put_left_off_btn = tkinter.Button(master=detector_grid_frame, text="PUT / LEFT / OFF",
                                              command=_save_put_left_off)
    save_as_put_right_dm_btn = tkinter.Button(master=detector_grid_frame, text="PUT / RIGHT / DOMIN.",
                                              command=_save_put_right_dm)
    save_as_put_left_dm_btn = tkinter.Button(master=detector_grid_frame, text="PUT / LEFT / DOMIN.",
                                             command=_save_put_left_dm)
    save_as_not_swing_btn = tkinter.Button(master=detector_grid_frame, text="NAV SITIENS",
...
if __name__ == "__main__":
    if len(sys.argv) <= 1:
        print("Provide a name of folder under ./e2e_dataset as command argument")
        exit(1)
